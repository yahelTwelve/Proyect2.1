...
const WeatherNight = ({ data }) => {
    ...
    return (
        <div className="w-screen h-screen flex items-center justify-center flex-col bg-cover" style={{ backgroundImage: `linear-gradient(to right, rgba(20, 16, 19, 0.4), rgba(20, 16, 19, 0.4)), ${fond}` }}>
            <article className="w-[280px] flex items-center justify-center flex-col gap-8 font-sans md:w-[450px] lg:w-[750px] lg:gap-[70px]" rgba>
                <h2 className="text-[18px] font-bold title text-gray-200 md:text-[25px]">{data.name}, {data.sys.country}</h2>
                <div className="flex flex-col w-[95%] gap-7 lg:flex-row lg:justify-center md:gap-14">
                    <div className="flex flex-col gap-4 items-center h-[200px] justify-center bg-[#101010c8]  p-1 rounded-2xl shadow-lg shadow-gray-400  md:h-[260px] md:justify-evenly lg:w-[700px] lg:h-[300px]">
                        <h3 className="text-gray-200 text-[17px] uppercase font-semibold md:text-[22px]">{data.weather[0].description}</h3>
                        <div className="flex justify-between items-center w-[83%]">
                            ...
                            <img src={`https://openweathermap.org/img/wn/${data.weather[0].icon}@2x.png`} alt="" className="bg-[#808080b9] rounded-2xl w-14 md:w-[200px] m-4" />
                        </div>
                    </div>
